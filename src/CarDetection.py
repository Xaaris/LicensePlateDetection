...
def extract_results(yolo_results):
    ...
    for i in range(len(yolo_results)):
        if yolo_results[i][0] == "car" or yolo_results[i][0] == "bus":
            x = int(yolo_results[i][1])
            y = int(yolo_results[i][2])
            w = int(yolo_results[i][3])
            h = int(yolo_results[i][4])
            upper_left = (x - w // 2, y - h // 2)
...
def detect_vehicle(image):
    ...
    yolo_results = extract_results(yolo.result_list)
